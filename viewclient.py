...
def enterType1Step1(section):
  ...
  for image in enumerateEnsureColor((345, 200), (72, 55, 38, 255), "对话框"):
    closeDialog(image)
  enterType1Step2()
...
def enterType1Step2():
  touch((700, 350), "困难", 1)
  touch((1450, 800), "探索", 5)
  for image in enumerateEnsureColor((1910, 75), (119, 70, 29, 255), "进入章节"):
    closeDialog(image)

def battleType1(section):
  times = 0
  for image in enumerateEnsureColor((1910, 100), (152, 110, 53, 255), "进入战场", 100):
    times += 1
    ...
    if compareColor(getColor((1500, 970), image), (45, 33, 26, 255), 100):
      touch((1500, 970), "自动准备")

    point = findColor((244, 225, 215, 255), region, image) # 首领
    if point:
      touch(point, "选择首领")
      continue
  ...
  for image in enumerateEnsureColor((1910, 75), (119, 70, 29, 255), "离开战场"):
    closeDialog(image)
    touch((1910, 75), "离开战场")
...
# print findColor((72, 55, 38, 255), ((0, 100), (1920, 500)))

# takeSnapshot().save("snapshots/1.png")
